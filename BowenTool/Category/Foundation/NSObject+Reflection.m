...
{
    return [[self class] ua_propertyKeys];
}
+ (NSArray *)ua_propertyKeys
{
...
{
    return [[self class] ua_propertiesInfo];
}
/**
...
+ (NSArray *)ua_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self ua_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)ua_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] ua_propertiesInfo];
...
-(NSArray *)ua_methodListInfo
{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++)
        {
            ...
            [arguments addObject:[[self class] ua_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] ua_decodeType:returnType];
        NSString *encodeString = [[self class] ua_decodeType:encoding];
...
{
    return [[self class]ua_protocolList];
}
+ (NSDictionary *)ua_protocolList
{
...
+ (NSArray *)ua_instanceVariable
{
    ...
    for (int i = 0; i < outCount; i++)
    {
        NSString *type = [[self class] ua_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)ua_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)ua_decodeType:(const char *)cString
{
    ...
    else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"])
        {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString ua_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
