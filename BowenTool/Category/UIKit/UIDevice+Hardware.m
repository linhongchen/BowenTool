...
{
    NSString *platform = [self ua_platform];
...
+ (NSUInteger)ua_getSysInfo:(uint)typeSpecifier
{
...
{
    return [self ua_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)ua_busFrequency
{
    return [self ua_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)ua_ramSize
{
    return [self ua_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)ua_cpuNumber
{
    return [self ua_getSysInfo:HW_NCPU];
}


+ (NSUInteger)ua_totalMemoryBytes
{
    return [self ua_getSysInfo:HW_PHYSMEM];
}
